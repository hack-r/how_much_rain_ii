# From Troy Walters on Kaggle (thanks!)
# A few small changes by Jason Miller, hack-r.com
...
results <- train.samp1 %>% group_by(Id = train.samp1) %>% summarize(Expected=mpalmer(train.samp1$Ref, minutes_past = train.samp1$minutes_past))

res1 <- mpalmer(ref = train.samp1$Ref, minutes_past = train.samp1$minutes_past)
